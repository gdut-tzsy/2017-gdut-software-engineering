...
$(document).ready(function(){
	...
	$(".search_Pop").on("click","li",function(){
		...
		var html = "<li id=\"property_title_liid\"><a href=\"javascript:changeSearchProperty('','')\">收件人</a></li>"
						...
						//+"<li id=\"property_title_liid\" class=\"cOneKey\" style=\"display:none\"><a href=\"javascript:oneKeyReminders()\">一键催领</a></li><li id=\"property_title_liid\" class=\"gOneKey\" style=\"display:none\"><a href=\"javascript:oneKeyClose()\">一键关闭</a></li>";
						+"<li id=\"property_title_liid\" class=\"cstatus1\" style=\"display:none\"><a href=\"javascript:changeSearchProperty('cstatus1','')\">已领取的</a></li><li id=\"property_title_liid\" class=\"cstatus0\" style=\"display:none\"><a href=\"javascript:changeSearchProperty('cstatus0','')\">未领取的</a></li>";
	...
	function enterSearch(){
		...
		pageIndex=1;
		$(".address_list").html('<div class="search-box-height"></div>');
		doSearch();
	}
	...
	function doSearch() {
		...
		$('#orginfo_search').ajaxSubmit({
			...
			success : function(result) {
				...
				if ("0" == result.code) {
					...
					if(taskList&&taskList.length>0){
						...
						$(".address_list").append('<div class="warp-no-data" id="warp-no-data" style=""> <i class="icon-no-data icon-kuaidi"></i><p class="mt20 c666 fz20">没有快递记录</p> <p class="f-color-aaa">手机扫码快速录入、一键催领</p></div>');
					}
				} 
			...
			if(result.data.ispower){//录入权限
				$(".cInput").show();
				$(".single-foot").show();//录入快递显示
				if($("#cstatus").val()=='0'){//未领取列表
					/*$(".cOneKey").show();//一键催领显示
					$(".gOneKey").show();//一键关闭*/
					$('.search_diandd').show();//一键催领、一键关闭显示
...
function changeSearchProperty(tips,valType){
	/*var html1 = "<li id=\"property_title_liid\"><a href=\"javascript:changeSearchProperty('','')\">收件人</a></li>"+
	"<li id=\"property_title_liid\"><a href=\"javascript:changeSearchProperty('cTime','')\">创建时间</a></li><li id=\"property_title_liid\" class=\"cInput\" style=\"display:none\"><a href=\"javascript:changeSearchProperty('cInput','')\">我录入的</a></li>"+
	"<li id=\"property_title_liid\" class=\"cOneKey\" style=\"display:none\"><a href=\"javascript:oneKeyReminders()\">一键催领</a></li><li id=\"property_title_liid\" class=\"gOneKey\" style=\"display:none\"><a href=\"javascript:oneKeyClose()\">一键关闭</a></li>";
	if(tips=="cTime"){
	...
	if("cTime"==tips){
		$("#ctype").val("1");
		$('.search_input').show().attr('name','keyWord');
		$('.search_Input').hide().attr('name','');
		...
		$('.search_Input').show().attr('name','keyWord');
		$("#ctype").val("2");
		$("#keyword").attr("placeholder","搜索收件人");
		if($("#cstatus").val()=='0'){
			$(".cstatus0").hide();
		}else if($("#cstatus").val()=='1'){
			$(".cstatus1").hide();
	...
	if("cstatus1"==tips){
		...
		window.location.href = baseURL+"/jsp/wap/express/list.jsp?status=1&agentCode=express&abc=1";
	}else if("cstatus0"==tips){
		$("#cstatus").val("0");
		$(".cstatus0").hide();
		document.title="拿快递";
		window.location.href = baseURL+"/jsp/wap/express/list.jsp?status=0&agentCode=express&abc=1";
	}
	/*$('.searchTitleIcon').on('click',function(){
	    if($(this).hasClass('faSortDown')){
	        $(this).removeClass('faSortDown').addClass('faSortUp');
...
function doSure(){
	...
	$.ajax({
		...
		success:function(result){
			...
			$("#btncuiB").hide();	//禁止多次点击催领快递
			$("#btncuiA").show();
	             _alert("提示","催领成功","确认",function(){restoreSubmit();});
			},
...
function closeExpress(){
	...
	$.ajax({
		...
		success:function(result){
			...
			$("#btncloseB").hide();	 //禁止多次点击关闭快递
			$("#btncloseA").show();
				 _alert("提示","关闭成功","确认",function(){enterSearch();restoreSubmit();});
...
function oneKeyReminders(){
	$('#qwui-actionsheet').removeClass('qwui-actionsheet_toggle');
	$('#iosMask').fadeOut(200);
	var length = $('#comments').children('div[class="settings-item"]').length;
	if(length<=0){
		_alert("","没有未领取的快递","确定",function(){restoreSubmit();});
		return;
	}
	var type=$("#ctype").val();
	var keyWord = $("#ckeyWord").val();
	 _alert("   ","给所有未领取快递的收件人发通知，确认催领？","取消|确定",{
		 ok:function(result){
					showLoading("正在发送...");
					$.ajax({
						...
						success:function(result){ 
							hideLoading();
				             ...
				             _alert("  ","催领成功","确定",function(){restoreSubmit();});
						},
						error:function(){ hideLoading();}
					});					
				},
...
function confirmOneKeyClose(){
	var length = $('#comments').children('div[class="settings-item"]').length;
	if(length<=0){
		 _alert("  ","没有未领取的快递","确认",function(){restoreSubmit();});
		return;
	}
	showLoading("请求中...");
//	var status=$("#cstatus").val();
